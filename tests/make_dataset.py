...
for data in tqdm(dataset["train"]):
    ...
    prompt = f"""
You are a hardware design expert. Given the following code description and corresponding Verilog code, provide a clear, concise, complete and coherent reasoning content explaining how you would approach solving the task of generating Verilog code based on the description.
- Code Description: {instruction}
- Verilog Code: {code}
"""

    response = DeepSeek(prompt)
    output = f"<think>\n{response}\n</think>\n\n<answer>\n```verilog\n{module_header}{module_code}\n```\n</answer>"
    
    json_dataset.append({"instruction": instruction, "input": "", "output": output})

with open("/root/autodl-tmp/ChipSeek-R1/data/mg-verilog.json", "w") as f:
