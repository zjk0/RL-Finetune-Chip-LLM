...
def ppa_compute(code_folder, verilog_code_file, module_name):
    ...
    try:
        ...
        if process.returncode != 0:
            print("Error while reading ppa_metrics.json:", process.stderr)
            return {"power": -1, "performance": -1, "area": -1}
...
for data in tqdm(dataset):
    top_module = auto_top(data["ground_truth"][0]["content"])
    ...
    with open(ground_truth_path, "w") as f:
        f.write(data["ground_truth"][0]["content"])
    ...
    if ppa_result["power"] != -1 and ppa_result["performance"] != -1 and ppa_result["area"] != -1:
        ...
        data_with_ppa = data.copy()
        data_with_ppa["ppa"] = ppa_result
        dataset_with_ppa.append(data_with_ppa)
        
table = pa.Table.from_pylist(dataset_filtered)
pq.write_table(table, "/root/autodl-tmp/ChipSeek-R1/data/codev_r1_rl/codev_r1_rl.parquet")
table = pa.Table.from_pylist(dataset_with_ppa)
pq.write_table(table, "/root/autodl-tmp/ChipSeek-R1/data/codev_r1_rl_ppa/codev_r1_rl_ppa.parquet")